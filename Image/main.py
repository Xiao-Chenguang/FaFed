...
from FAFED import FAFED
from FedAvg import FedAvg
from net import CIFARModel, FashionMNISTModel, MNISTModel
...
def partition_dataset(args):
    ...
    if args.dataset == "mnist1":
        ...
        dataset = DISTCIFAR10(
            "./cifar10d/", rank=rank, train=True, download=True, transform=transform_c
        )
        testset = DISTCIFAR10(
            "./cifar10d/", rank=rank, train=False, download=True, transform=transform_c
        )
    elif args.dataset == "cifar2":
        dataset = HiDISTCIFAR10(
            "./cifar10h/", rank=rank, train=True, download=True, transform=transform_c
        )
        testset = HiDISTCIFAR10(
            "./cifar10dh/", rank=rank, train=False, download=True, transform=transform_c
        )
    else:
...
def run(rank, size, args):
    ...
    if (
        ...
        or args.dataset == "mnist1"
        or args.dataset == "mnist2"
    ):
        model = MNISTModel()
    elif (
        args.dataset == "fmnist0"
        or args.dataset == "fmnist1"
        or args.dataset == "fmnist2"
    ):
        model = FashionMNISTModel()
    elif (
        args.dataset == "cifar0" or args.dataset == "cifar1" or args.dataset == "cifar2"
    ):
        model = CIFARModel()
    else:
        print("ERRORRRR")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--inLoop", type=int, default=10, metavar="S", help="inter loop number"
    )
    parser.add_argument(
        "--init",
        action="store_true",
