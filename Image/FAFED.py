...
def FAFED(train_set, test_set, model, args, device):
    ...
    for epoch in range(args.epochs):
        for siter, (data, target) in enumerate(train_set):
            ...
            # Update
            for i, (para1, para2) in enumerate(
                zip(model.parameters(), model_.parameters())
            ):
                m_t[i] = para1.grad.data + (1 - args.alpha) * (
                    m_t[i] - args.ext * para2.grad.data
                )
            ...
            if Iteration % args.inLoop == 0:
                ...
                loss_value = torch.tensor(
                    [loss.item(), correct_cnt, total_cnt], dtype=torch.float64
                )
                ...
                if rank == 0:
                    ...
                    print(
                        Iteration // args.inLoop,
                        loss_lst[0] / size,
                        loss_lst[1] / loss_lst[2],
                    )
                ###########
            Iteration += 1
