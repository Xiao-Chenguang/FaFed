...
def run(rank, size, args):
    ...
    model = RNNModel(
        "LSTM", ntokens, args.emsize, args.nhid, args.nlayers, args.dropout
    ).to(device)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--emsize", type=int, default=650, help="size of word embeddings"
    )
    parser.add_argument(
        "--nhid", type=int, default=650, help="number of hidden units per layer"
    )
    ...
    parser.add_argument(
        "--ext", type=float, default=1, metavar="alpha", help="momentum rate beta"
    )
    parser.add_argument(
